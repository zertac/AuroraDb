...
namespace AuroraDb.Connectors
{
    internal class AMSqlConnector
    {
        ...
        public AMSqlConnector(string cnnStr = "")
        {
        ...
        private void OpenConnection(string cnnStr)
        {
            try
            {
                _dnCon = new SqlConnection(cnnStr);
        ...
        {
            var query = DataQueries.Queries[ConnectionTypes.SQL_SERVER][queryKey];
            ...
            if (values != null)
            {
                ...
                {
                    var p = new SqlParameter(v.key, SqlDbType.VarChar)
                    {
                        Value = v.value,
